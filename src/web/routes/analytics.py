...
async def analytics_page(
    request: Request, tab: str = Query("personal"), auth: tuple = Depends(require_login)
):
    user, session = auth
    try:
        ...
        if user.is_admin:
            ...
            # System-level stats
            total_articles = session.exec(select(sqlfunc.count(Article.id))).one() or 0
            processed_articles = (
                session.exec(
                    select(sqlfunc.count(Article.id)).where(
            ...
            total_embeddings = (
                session.exec(select(sqlfunc.count(ArticleEmbedding.id))).one() or 0
            )
            total_all_impressions = (
                session.exec(select(sqlfunc.count(FeedImpression.id))).one() or 0
            )
            ...
            avg_alpha = (
                round(sum(alpha_values) / len(alpha_values), 2) if alpha_values else 1.0
            )
